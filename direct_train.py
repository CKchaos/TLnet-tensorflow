...
with tf.device(dev_id):
   y_hat_2d = tlnet(x , imgs, is_training=True)

loss = tlnet.get_loss(x,y_hat_2d)

with tf.device(dev_id):
    optimiter = tf.train.MomentumOptimizer(learning_rate=lr, momentum=0.9)
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    with tf.control_dependencies(update_ops):
        opt = optimiter.minimize(loss)

tf.summary.scalar("tl_net/total_loss", loss)
...
with tf.Session(config=tf_config) as sess:
    ...
    for i in range( max_itrs):
        ...
        if i % 20 == 0:
            ...
            result = sess.run((loss, merged), feed_dict=feed_dict)
            summary = result[-1]
            print("\033[1;32mTOTAL LOSS:\t\033[1;33m{:.8f}\033[0m\n".format(result[0]))
