...
class TLNet():
    ...
    def __call__(self, x, imgs, is_training, reuse=False):
       ...
       z_3d = self._encoder(x, is_training, reuse=reuse)
       y_hat_3d = self._decoder(z_3d, is_training, reuse=reuse)
       y_hat_2d = self._decoder(z_2d, is_training, reuse=True)
       return z_2d, z_3d, y_hat_3d, y_hat_2d
    ...
    def _encoder(self, x, is_training, reuse=False):
        with tf.variable_scope("encoder",reuse = reuse):
            ...
            W_conv1 = tf.get_variable('e_wconv1', [4, 4, 4, 1, 256], initializer=tf.truncated_normal_initializer(stddev=0.02))
            b_conv1 = tf.get_variable('e_bconv1', [256], initializer=tf.constant_initializer(0))
            h_conv1 = tf.nn.conv3d(input=x, filter=W_conv1, strides=[1, 2, 2, 2, 1], padding='SAME') + b_conv1
            h_norm1 = tf.contrib.layers.batch_norm(inputs = h_conv1, center=True, scale=True, is_training=is_training, scope="e_bn1")
            h_relu1 = tf.nn.relu(h_norm1)

            W_conv2 = tf.get_variable('e_wconv2', [4, 4, 4, 256, 128], initializer=tf.truncated_normal_initializer(stddev=0.02))
            b_conv2 = tf.get_variable('e_bconv2', [128], initializer=tf.constant_initializer(0))
            h_conv2 = tf.nn.conv3d(input=h_relu1, filter=W_conv2, strides=[1, 2, 2, 2, 1], padding='SAME') + b_conv2
            h_norm2 = tf.contrib.layers.batch_norm(inputs = h_conv2, center=True, scale=True, is_training=is_training, scope="e_bn2")
            h_relu2 = tf.nn.relu(h_norm2)

            W_conv3 = tf.get_variable('e_wconv3', [4, 4, 4, 128, 64], initializer=tf.truncated_normal_initializer(stddev=0.02))
            b_conv3 = tf.get_variable('e_bconv3', [64], initializer=tf.constant_initializer(0))
            h_conv3 = tf.nn.conv3d(input=h_relu2, filter=W_conv3, strides=[1, 2, 2, 2, 1], padding='SAME') + b_conv3
            h_norm3 = tf.contrib.layers.batch_norm(inputs = h_conv3, center=True, scale=True, is_training=is_training, scope="e_bn3")
            h_relu3 = tf.nn.relu(h_norm3)

            W_conv4 = tf.get_variable('e_wconv4', [4, 4, 4, 64, self.z_dim], initializer=tf.truncated_normal_initializer(stddev=0.02))
            b_conv4 = tf.get_variable('e_bconv4', [self.z_dim], initializer=tf.constant_initializer(0))
            h_conv4 = tf.nn.conv3d(input=h_relu3, filter=W_conv4, strides=[1, 1, 1, 1, 1], padding='VALID') + b_conv4
            h_norm4 = tf.contrib.layers.batch_norm(inputs = h_conv4, center=True, scale=True, is_training=is_training, scope="e_bn4")
            h_relu4 = tf.nn.relu(h_norm4)
    ...
    def get_loss(self, x, z_2d, z_3d, y_hat_3d, y_hat_2d):

        z_3d_stopped = tf.stop_gradient(z_3d)
        z_loss_stopped = tf.nn.l2_loss(z_2d - z_3d_stopped)
        x_loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=x,logits=y_hat_3d))

        z_loss_total = (1.0/self.batch_size) * self.lambda_z * tf.nn.l2_loss(z_2d - z_3d)
        loss = [self.lambda_z * z_loss_stopped, self.lambda_x * x_loss]

        loss = tuple([(1.0/self.batch_size) * x for x in loss])
        total_loss = z_loss_total + loss[1]
